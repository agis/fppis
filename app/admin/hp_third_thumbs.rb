ActiveAdmin.register HpThirdThumb do
  menu :parent => "Homepage", :label => 'Thumb #3 Area'
  ...
  form do |f|
    f.inputs "Images", :multipart => true do
      ...
      f.input :preview, :as => :third_thumb
    end
  ...
  controller do
    def show
      show! do |format|
        format.html { redirect_to admin_hp_third_thumbs_path }
      end
    end
  end

  sidebar " " do
    raise "More than one Homepage resources detected. Contact the developer please." if HpThirdThumb.count > 1
    link_to 'Edit Images', edit_admin_hp_third_thumb_path(HpThirdThumb.first)
  end
end